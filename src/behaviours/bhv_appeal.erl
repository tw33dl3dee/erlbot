...
handle_appeal(Cause, Source, ?USER(Nick) = User, Msg) ->
	...
	case appeal_react(AppealType, Msg, IsBot, Source, Nick, Cause) of
...
appeal_react(kiss, _, _, _, _, _)                     -> {message_react, action,  ["*KISSED* *YAHOO*"]};
appeal_react(humiliation, _, _, _, Nick, _)           -> {message_react, chanmsg, [Nick, ": хамишь, сцуко."]};
appeal_react(baran, _, _, _, Nick, _)                 -> {message_react, chanmsg, [Nick, ": сам баран, баран!!!!"]};
appeal_react(greeting, _, _, _, Nick, _)              -> {message_react, chanmsg, ["\\O/ Превед, ", Nick, "!!!"]};
appeal_react(caress, _, _, _, _, _)                   -> {message_react, chanmsg, "^_^"};
appeal_react(criticism, _, _, _, _, _)                -> {message_react, action,  "тупая пизда v_v"};
appeal_react(fuckoff, _, false, Chan, Nick, _)
  when ?IS_CHAN(Chan)                                 -> {delayed_event, ?APPEAL_DELAY, customevent,
														  {suicide, Chan, Nick}, undefined};
% Other bots cannot kick
appeal_react(fuckoff, _, true, _, Nick, _)            -> {message_react, chanmsg, [Nick, ": слишком много чести."]};
appeal_react(_, Msg, false, Source, _, direct)        -> {new_event, customevent, {markov, Source, Msg}, undefined};
appeal_react(_, _Msg, _IsBot, _Source, _Nick, _Cause) -> not_handled.
