...
%%% port           | port to connect to                                               | 6667
%%% ping_timeout   | maximum timeout in msec between pings from server                | 90000
%%% sock_timeout   | maximum timeout for operations on socket                         | 60000
%%% maxsend        | maximum line length to send (BUG: should be 512, workaround)     | 450
%%% conn_rate      | maximum connection rate ({Connections, MSecs})                   | {2, 16000}
%%%
%%%-------------------------------------------------------------------

-define(DEFAULT_PORT,         6667).
-define(DEFAULT_PING_TIMEOUT, 90000).
-define(DEFAULT_SOCK_TIMEOUT, 60000).
-define(DEFAULT_MAXSEND,      450).
-define(DEFAULT_CONN_RATE,    {2, 16000}).
...
init(Owner) ->
	{ok, #state{owner = Owner, 
				host    = erlbot_config:get_value(host, required), 
				ping    = erlbot_config:get_value(ping_timeout, ?DEFAULT_PING_TIMEOUT), 
				maxsend = erlbot_config:get_value(maxsend, ?DEFAULT_MAXSEND)}}.
...
do_connect(#state{host = Host} = State) ->
	...
	conn_throttle(State),
	Port        = erlbot_config:get_value(port, ?DEFAULT_PORT),
	SockTimeout = erlbot_config:get_value(sock_timeout, ?DEFAULT_SOCK_TIMEOUT),
...
conn_throttle(#state{host = Host}) ->
	{MaxConn, Period} = erlbot_config:get_value(conn_rate, ?DEFAULT_CONN_RATE),
